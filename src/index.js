...
class EGPlugin {
	constructor(serverless, options) {
		...
		this.hooks = {
			'package:initialize': this.createConnectorFunctionDefinitions.bind(
				this
			),
		...
		this.commands = {
			gateway: {
				...
				commands: {
					...
					dashboard: {
						usage:
							'Show functions and subscriptions in Space for Event Gateway',
	...
	createConnectorFunctionDefinitions() {
		...
		for (let name in functions) {
			if (!functions.hasOwnProperty(name) || !functions[name].type)
				continue;
			const func = (this.connectorFunctions[name] = functions[name]);

			if (!func.inputs)
				throw new Error(
					`No inputs provided for ${func.type} function "${name}".`
				);

			if (!['awsfirehose', 'awskinesis', 'awssqs'].includes(func.type)) {
				throw new Error(
					`Unrecognised type "${func.type}" for function "${name}"`
				);
			}

			const { resourceName, action } = this.connectorFunctionNames(
				name,
				func.type
			);
			if (
				!(
					...
					(func.inputs.hasOwnProperty('arn') &&
						func.inputs.hasOwnProperty(resourceName))
				)
			) {
				throw new Error(
			...
			if (func.inputs.hasOwnProperty('logicalId')) {
				if (
					!this.serverless.service.resources.Resources.hasOwnProperty(
					...
					throw new Error(
						`Could not find resource "${
							func.inputs.logicalId
						}" in "resources" block for "${name}" function.`
					);
				}
				...
				this.connectorFunctionsOutputs = Object.assign(
					this.connectorFunctionsOutputs,
					this.connectorFunctionOutput(name, func.type, {
						logicalId: func.inputs.logicalId
					})
				);
		...
		if (subscriptions instanceof Array && subscriptions.length) {
			const unsubList = subscriptions.map(sub =>
				eg
					...
					.then(() => {
						this.serverless.cli.consoleLog(
							`EventGateway: Subscription "${
								sub.event
							}" removed from function: ${sub.functionId}`
						);
					})
			);
		...
		if (Array.isArray(functions) && functions.length) {
			const deleteList = functions.map(func =>
				eg.deleteFunction({ functionId: func.functionId }).then(() => {
					this.serverless.cli.consoleLog(
						`EventGateway: Function "${func.functionId}" removed.`
					);
				})
			);
	...
	async configureEventGateway() {
		...
		if (!stack) {
			throw new Error(
				'Unable to fetch CloudFormation stack information.'
			);
		}
		...
		if (
			...
			this.connectorFunctions.length === 0
		) {
			return;
		}
		...
		this.filterFunctionsWithEvents().map(async name => {
			...
			if (!registeredFunction) {
				...
				this.serverless.cli.consoleLog(
					`EventGateway: Function "${name}" registered. (ID: ${
						fn.functionId
					})`
				);
				...
				let existingSubscriptions = registeredSubscriptions.filter(
				...
				const subscriptionsToDelete = existingSubscriptions.map(sub =>
					eg
						...
						.then(() =>
							this.serverless.cli.consoleLog(
								`EventGateway: Function "${name}" unsubscribed from "${
									sub.event
								}" event.`
							)
						)
		...
		for (let name in this.connectorFunctions) {
			...
			const cfId = `${
				this.serverless.service.service
			}-${this.awsProvider.getStage()}-${name}`;
			const registeredFunction = registeredFunctions.find(
				f => f.functionId === cfId
			);
			registeredFunctions = registeredFunctions.filter(
			...
			if (!registeredFunction) {
				...
				this.serverless.cli.consoleLog(
					`EventGateway: Function "${name}" registered. (ID: ${cfId})`
				);
				...
				const events = cf.events
					...
					.map(eventObj =>
						this.createSubscription(
							config,
							cfId,
							eventObj.eventgateway
						).then(() =>
							this.serverless.cli.consoleLog(
								`EventGateway: Function "${name}" subscribed for "${
									eventObj.eventgateway.event
								} event.`
							)
						)
					);
		...
		registeredFunctions.forEach(async functionToDelete => {
			...
			await Promise.all(
				subscriptionsToDelete.map(toDelete =>
					eg.unsubscribe({ subscriptionId: toDelete.subscriptionId })
				)
			);

			await eg.deleteFunction({
				functionId: functionToDelete.functionId
			});
			this.serverless.cli.consoleLog(
				`EventGateway: Function "${
					functionToDelete.functionId
				}" and it's subscriptions deleted.`
			);
		});
	}
	...
	async registerConnectorFunction(name, func, funcId) {
		...
		const expectedOutputName = this.connectorFunctionNames(name, func.type)
			.outputName;
		...
		switch (func.type) {
			case 'awsfirehose':
				if (
					func.inputs.hasOwnProperty('arn') &&
					func.inputs.hasOwnProperty('deliveryStreamName')
				) {
					fn.provider.deliveryStreamName =
						func.inputs.deliveryStreamName;
				} else if (func.inputs.hasOwnProperty('logicalId')) {
					if (!this.outputs[expectedOutputName]) {
						throw new Error(
							`Expected "${expectedOutputName}" in Stack Outputs but not found`
						);
					}
					fn.provider.deliveryStreamName = this.outputs[
						expectedOutputName
					];
				}
				break;
			case 'awskinesis':
				if (
					func.inputs.hasOwnProperty('arn') &&
					func.inputs.hasOwnProperty('streamName')
				) {
			...
			case 'awssqs':
				if (
					func.inputs.hasOwnProperty('arn') &&
					func.inputs.hasOwnProperty('queueUrl')
				) {
		...
		if (err) {
			throw new Error(
				`Couldn't register Connector Function "${name}": ${err}`
			);
		}
		return result;
	}
	...
	async registerFunction(fn) {
		...
		if (err) {
			throw new Error(
				`Couldn't register a function ${fn.functionId}. ${err}.`
			);
		}
		return result;
	}
	...
	async createSubscription(config, functionId, event) {
		...
		if (err) {
			throw new Error(
				`Couldn't create subscriptions for ${functionId}. ${err}.`
			);
		}
		return result;
	}
