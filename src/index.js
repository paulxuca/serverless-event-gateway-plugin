  ...
  async getEGServiceFunctions() {
    ...
    return functions.filter(f => {
      return f.functionId.startsWith(
        `${this.awsProvider.getRegion()}-${this.serverless.service.service}`
      );
    });
  }
    ...
    if (subscriptions instanceof Array && subscriptions.length) {
      const unsubList = subscriptions.map(sub =>
        eg.unsubscribe({ subscriptionId: sub.subscriptionId }).then(() => {
          this.serverless.cli.consoleLog(
            `EventGateway: Subscription "${sub.event}" removed from function: ${
              sub.functionId
            }`
          );
        })
  ...
  async configureEventGateway() {
    ...
    this.filterFunctionsWithEvents().map(async name => {
      ...
      if (!registeredFunction) {
        ...
        this.serverless.cli.consoleLog(
          `EventGateway: Function "${name}" registered. (ID: ${fn.functionId})`
        );
    ...
    this.serverless.cli.consoleLog(
      `EventGateway: deleting below functions and their subscriptions.`
    );
    this.serverless.cli.consoleLog(
      JSON.stringify(registeredFunctions, null, 2)
    );
    ...
    registeredFunctions.forEach(async functionToDelete => {
      ...
      await Promise.all(
        subscriptionsToDelete.map(async toDelete => {
          await eg.unsubscribe({ subscriptionId: toDelete.subscriptionId });

          this.serverless.cli.consoleLog(
            `EventGateway: Subscription ${toDelete.subscriptionId} deleted.`
          );
        })
      );
